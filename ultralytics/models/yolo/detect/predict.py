...
class DetectionPredictor(BasePredictor):
    ...
    def postprocess(self, preds, img, orig_imgs, **kwargs, img_embeddings):
        """Post-processes predictions and returns a list of Results objects."""
        preds, bbox_embeddings = ops.non_max_suppression(
            preds,
            self.args.conf,
            self.args.iou,
            self.args.classes,
    ...
    def construct_result(self, pred, img, orig_img, img_path):
        ...
        return Results(orig_img, path=img_path, names=self.model.names, boxes=pred, img_embeddings = img_embeddings, bbox_embeddings = bbox_embeddings[:, :6])
